...
export const GhostCursor = ({
  ...
  brightness = 1,
  color = '#22d3ee', // Using cyan-400 to match the theme
  mixBlendMode = 'screen',
  edgeIntensity = 0,
  ...
  const fragmentShader = `
    ...
    void main() {
      // Use vUv directly for perfectly normalized coordinates
      ...
      for (int i = 0; i < MAX_TRAIL_LENGTH; i++) {
        ...
        if (t > 0.01) {
          ...
          // Use max() instead of += to prevent blinding blowout when mouse is stationary
          colorAcc = max(colorAcc, bt.rgb);
          alphaAcc = max(alphaAcc, bt.a);
        }
      }

      colorAcc *= iBrightness;
      
      // Use vUv for perfect edge distance masking
